...
def whatsapp_call_ui(shop_phone="", shop_context="", check_sub=False, expiry_date="2099-01-01"):
    ...
    if check_sub:
        try:
            ...
            if datetime.date.today() > exp:
                st.markdown("""
                <div style='background:rgba(255,59,48,0.08);border:1px solid rgba(255,59,48,0.25);
                     border-radius:16px;padding:24px;text-align:center;'>
                  <p style='font-size:1.5rem;margin-bottom:8px;'>🔒</p>
                  <p style='color:#ff5c6e;font-weight:700;font-size:.95rem;margin-bottom:4px;'>মেয়াদ শেষ হয়েছে</p>
                  <p style='color:rgba(255,150,150,0.5);font-size:.78rem;'>Secret Key দিয়ে মেয়াদ বাড়ান</p>
                </div>
    ...
    # ── voice query param থেকে AI উত্তর ──────────────────
    qp = st.query_params
    vr = qp.get("vr", "").strip()
    if vr and vr != "None":
    ...
    if st.session_state.call_active and not st.session_state.call_greeted:
        with st.spinner("সংযুক্ত হচ্ছে..."):
            greet = get_ai_sales_response(
                "কাস্টমার এইমাত্র কল করেছে। আগে আসসালামু আলাইকুম বলো, তারপর কাস্টমার কেয়ার থেকে বলছি বলে পরিচয় দাও এবং কীভাবে সাহায্য করতে পারো জিজ্ঞেস করো।",
                shop_context
            )
...
(function(){{
  ...
  try {{
    ...
    ctx.decodeAudioData(buf, function(decoded){{
      ...
      src.onended = function(){{ startListen(); }};
      src.start(0);
    ...
    # ── HTML UI ───────────────────────────────────────────
    CALL_ICON = """<svg viewBox="0 0 24 24" width="30" height="30" fill="white"><path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1-9.4 0-17-7.6-17-17 0-.6.4-1 1-1h3.5c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.3 0 .7-.2 1L6.6 10.8z"/></svg>"""
    END_ICON  = """<svg viewBox="0 0 24 24" width="30" height="30" fill="white"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.12-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z"/></svg>"""

    av_cls   = "av ring" if not is_active else "av live"
    status_t = "কল করতে নিচের সবুজ বাটন চাপুন" if not is_active else "🟢 সংযুক্ত — কথা বলুন"
    call_btn = f'<button class="bcall" id="bcall" onclick="doCall()">{CALL_ICON}</button>' if not is_active else ""
    end_btn  = f'<button class="bend" onclick="doEnd()">{END_ICON}</button>' if is_active else ""
    timer_el = '<div class="tmr" id="tmr">00:00</div>' if is_active else ""
    pulse_el = '<div class="pls"><span></span><span></span><span></span></div>' if is_active else ""
...
body{{font-family:"Noto Sans Bengali",sans-serif;background:transparent;display:flex;justify-content:center;padding:8px 0;}}
.wrap{{display:flex;flex-direction:column;align-items:center;
  background:linear-gradient(180deg,#0d1117,#161b22);
  border-radius:24px;padding:28px 20px 22px;width:100%;max-width:320px;
...
.av{{width:86px;height:86px;border-radius:50%;
  ...
  display:flex;align-items:center;justify-content:center;font-size:2.6rem;margin-bottom:12px;}}
...
.st{{color:rgba(255,255,255,.4);font-size:.78rem;margin-bottom:16px;text-align:center;}}
.tmr{{color:rgba(255,255,255,.3);font-size:.78rem;margin-bottom:12px;font-variant-numeric:tabular-nums;}}
.pls{{display:flex;gap:5px;margin-bottom:14px;align-items:center;}}
...
.bcall{{width:68px;height:68px;border-radius:50%;background:#25d366;border:none;cursor:pointer;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 20px rgba(37,211,102,.6);animation:pg 1.7s ease infinite;}}
...
.bend{{width:68px;height:68px;border-radius:50%;background:#ff3b30;border:none;cursor:pointer;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 18px rgba(255,59,48,.5);}}
.bend:active{{transform:scale(.93);}}
.lbl{{color:rgba(255,255,255,.2);font-size:.68rem;margin-top:7px;}}
</style></head><body>
<div class="wrap">
  <div class="{av_cls}">🏪</div>
  <div class="nm">কাস্টমার কেয়ার</div>
  <div class="st">{status_t}</div>
  {timer_el}
  {pulse_el}
  {call_btn}
  {end_btn}
  <div class="lbl">{"📞 কল করুন" if not is_active else "📵 কল কাটুন"}</div>
</div>
<script>
var IS = {str(is_active).lower()};
var rec = null, sec = 0, tid = null;

function getBase(){{
  try {{ return window.top.location.href.split("?")[0]; }}
...
function doCall(){{
  var btn = document.getElementById("bcall");
  ...
  try{{
    ...
    function tone(f, s, d){{
      ...
      var t=ctx.currentTime+s;
      g.gain.setValueAtTime(0,t); g.gain.linearRampToValueAtTime(.13,t+.06);
      g.gain.setValueAtTime(.13,t+d-.06); g.gain.linearRampToValueAtTime(0,t+d);
      o.start(t); o.stop(t+d+.1);
    }}
    function ring(){{ tone(440,0,.42); tone(480,.04,.42); tone(440,.65,.42); tone(480,.69,.42); }}
    ring();
    window._rt = setInterval(function(){{
      var c2 = new(window.AudioContext||window.webkitAudioContext)();
      function t2(f,s,d){{
        var o=c2.createOscillator(),g=c2.createGain();
        o.connect(g);g.connect(c2.destination);o.type="sine";o.frequency.value=f;
        var t=c2.currentTime+s;
        g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(.13,t+.06);
        g.gain.setValueAtTime(.13,t+d-.06);g.gain.linearRampToValueAtTime(0,t+d);
        o.start(t);o.stop(t+d+.1);
      }}
      t2(440,0,.42);t2(480,.04,.42);t2(440,.65,.42);t2(480,.69,.42);
    }}, 2800);
  }}catch(e){{}}
...
function sendVoice(txt){{
  if(rec){{ try{{rec.abort();}}catch(e){{}} rec=null; }}
  nav(getBase() + "?vr=" + encodeURIComponent(txt));
}}
...
function startListen(){{
  ...
  try{{
    ...
    rec.onresult = function(e){{
      ...
      if(txt.trim()) sendVoice(txt.trim());
    }};
    rec.onend = function(){{
      if(IS) setTimeout(startListen, 1000);
    }};
...
if(IS){{
  tid = setInterval(function(){{
    ...
    var e=document.getElementById("tmr"); if(e) e.textContent=m+":"+s;
  }}, 1000);
}}
</script>
{audio_script}
    ...
    _c.html(html, height=340, scrolling=False)
...
VOICE_API_URL = os.environ.get("VOICE_API_URL", "https://global-ai-shop.onrender.com:8000")
...
def customer_page(shop_phone):
    ...
    try:
        ...
        is_expired = datetime.date.today() > exp
    except:
        is_expired = False
    ...
    prod_lines = []
    for p in products:
        prod_lines.append(p.get("name","") + ": " + p.get("desc",""))
    ctx = f"দোকান: {biz_name}. নিয়ম: {rules[:150]}. পণ্য: {chr(10).join(prod_lines[:15])}"
    ctx_safe = ctx.replace("\\","\\\\").replace("`","'").replace("$","")

    biz_safe = biz_name.replace("<","").replace(">","").replace("`","").replace("$","")
    voice_url = VOICE_API_URL
...
<div class="box">
  ...
  <div style="color:rgba(255,150,150,.5);font-size:.8rem;">ব্যবসায়ীকে জানান — Secret Key দিয়ে মেয়াদ বাড়াতে হবে।</div>
...
<title>""" + biz_safe + """</title>
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+Bengali:wght@400;700&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
html,body{height:100%;background:#010409;}
body{font-family:"Noto Sans Bengali",sans-serif;display:flex;align-items:center;
justify-content:center;min-height:100vh;
background-image:radial-gradient(ellipse 70% 50% at 50% 0%,rgba(37,211,102,.09) 0%,transparent 60%);}
.card{background:linear-gradient(180deg,#0d1117,#161b22);border-radius:28px;
padding:36px 24px 30px;width:92%;max-width:340px;
box-shadow:0 16px 64px rgba(0,0,0,.8);border:1px solid rgba(255,255,255,.07);
display:flex;flex-direction:column;align-items:center;}
.shop{color:rgba(255,255,255,.28);font-size:.7rem;letter-spacing:2px;
text-transform:uppercase;margin-bottom:22px;}
.av{width:94px;height:94px;border-radius:50%;
background:linear-gradient(135deg,#25d366,#075e35);
display:flex;align-items:center;justify-content:center;font-size:3rem;margin-bottom:14px;}
...
.nm{color:#fff;font-size:1.25rem;font-weight:700;margin-bottom:6px;}
.st{min-height:22px;font-size:.78rem;color:rgba(255,255,255,.4);margin-bottom:18px;
display:flex;align-items:center;gap:6px;text-align:center;}
.dot{width:8px;height:8px;border-radius:50%;background:#25d366;display:none;
animation:bk .9s infinite alternate;}
@keyframes bk{from{opacity:.2;}to{opacity:1;}}
.tmr{color:rgba(255,255,255,.28);font-size:.76rem;margin-bottom:12px;
font-variant-numeric:tabular-nums;display:none;}
.pulse{display:none;gap:6px;margin-bottom:14px;}
.pulse span{width:7px;height:7px;border-radius:50%;background:#25d366;
...
@keyframes pl{from{opacity:.15;transform:scale(.5);}to{opacity:1;transform:scale(1.5);}}
.ltxt{color:#25d366;font-size:.74rem;display:none;margin-bottom:10px;
animation:bk .9s infinite alternate;}
...
svg{width:30px;height:30px;fill:white;pointer-events:none;}
.lbl{color:rgba(255,255,255,.2);font-size:.68rem;margin-top:8px;}
</style>
</head>
<body>
<div class="card">
  <div class="shop">""" + biz_safe + """</div>
  <div class="av" id="av">&#127978;</div>
  <div class="nm">&#2453;&#2494;&#2488;&#2509;&#2463;&#2478;&#2494;&#2480; &#2453;&#2503;&#2527;&#2494;&#2480;</div>
  <div class="st" id="st">
    <span class="dot" id="dot"></span>
    <span id="stxt">&#2453;&#2482; &#2453;&#2480;&#2468;&#2503; &#2472;&#2495;&#2458;&#2503;&#2480; &#2476;&#2494;&#2463;&#2472; &#2458;&#2494;&#2474;&#2497;&#2472;</span>
  </div>
  <div class="tmr" id="tmr">00:00</div>
  <div class="pulse" id="pulse"><span></span><span></span><span></span></div>
  <div class="ltxt" id="ltxt">&#2474;&#2494;&#2447;&#2479;&#2492;&#2472; &#2476;&#2482;&#2497;&#2472;...</div>
  ...
  <div class="lbl" id="lbl">&#9742;&#65039; &#2453;&#2482; &#2453;&#2480;&#2497;&#2472;</div>
</div>
<script>
var VAPI="__VOICE_API__";
var CTX="__CTX__";
var active=false,rec=null,sec=0,tid=null,busy=false,audioCtx=null,ringTmr=null;
function $(i){return document.getElementById(i);}
function setSt(t,d){$("stxt").textContent=t;$("dot").style.display=d?"inline-block":"none";}
...
function playB64(b64,cb){
  ...
  c.decodeAudioData(buf,function(dec){
    var s=c.createBufferSource();s.buffer=dec;s.connect(c.destination);
    s.onended=function(){busy=false;showP(false);if(active){setSt("বলুন...",true);showL(true);startListen();}if(cb)cb();};
    s.start(0);
  },function(){busy=false;if(active)startListen();if(cb)cb();});
}
async function apiCall(ep,msg){
  setSt("ভাবছে...",true);showL(false);
  try{
    var r=await fetch(VAPI+"/"+ep,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:msg,shop_context:CTX})});
    return await r.json();
  }catch(e){return {reply:"জ্বি বলুন।",audio:""};}
}
function speak(data){
  busy=true;showP(true);showL(false);setSt("বলছে...",true);
  if(data.audio&&data.audio.length>100){
    playB64(data.audio,null);
  } else if(window.speechSynthesis){
    window.speechSynthesis.cancel();
    var u=new SpeechSynthesisUtterance(data.reply);u.lang="bn-BD";u.rate=0.9;u.volume=1;
    var vs=window.speechSynthesis.getVoices(),bn=vs.find(function(v){return v.lang&&v.lang.startsWith("bn");});
    if(bn)u.voice=bn;
    ...
    var fx=setInterval(function(){if(!window.speechSynthesis.speaking)clearInterval(fx);else window.speechSynthesis.resume();},4000);
    window.speechSynthesis.speak(u);
  } else {busy=false;if(active)startListen();}
}
function startListen(){
  if(!active||busy)return;
  var SR=window.SpeechRecognition||window.webkitSpeechRecognition;
  if(!SR){setSt("Chrome এ খুলুন",false);return;}
  try{
    if(rec){try{rec.abort();}catch(e){}}
    rec=new SR();rec.lang="bn-BD";rec.continuous=false;rec.interimResults=false;rec.maxAlternatives=1;
    rec.onstart=function(){setSt("বলুন...",true);showL(true);};
    rec.onresult=function(e){
      showL(false);var txt="";
      for(var i=e.resultIndex;i<e.results.length;i++)if(e.results[i].isFinal)txt+=e.results[i][0].transcript;
      if(txt.trim()){setSt("বুঝছি...",true);apiCall("chat",txt.trim()).then(speak);}
    };
    rec.onend=function(){showL(false);if(active&&!busy)setTimeout(startListen,700);};
    rec.onerror=function(ev){showL(false);if(ev.error!=="aborted"&&ev.error!=="no-speech")setSt("মাইক চালু করুন",false);if(active&&!busy)setTimeout(startListen,1200);};
    rec.start();
...
function endCall(){
  ...
  $("av").className="av";$("bend").style.display="none";$("bcall").style.display="flex";
  $("tmr").style.display="none";showP(false);showL(false);
  setSt("কল শেষ হয়েছে",false);$("lbl").textContent="📞 আবার কল করুন";sec=0;
}
if(window.speechSynthesis){window.speechSynthesis.getVoices();window.speechSynthesis.onvoiceschanged=function(){};}
</script>
</body></html>"""

    html = html.replace("__VOICE_API__", voice_url).replace("__CTX__", ctx_safe)
    _cv1.html(html, height=600, scrolling=False)
...
def auth_page():
    ...
    with c:
        ...
        with t2:
            ...
            st.markdown("""
            <div style='background:rgba(251,191,36,0.07);border:1px solid rgba(251,191,36,0.2);border-radius:10px;padding:10px 14px;margin:10px 0;'>
...
def main():
    ...
    if "shop" in qp:
        ...
        return
    # Dashboard এ থাকা অবস্থায় call/vr param এলে logged_in check ছাড়াই dashboard চালাও
    if st.session_state.get("logged_in") and (
        "call" in qp or "vr" in qp
    ):
        dashboard()
        return
