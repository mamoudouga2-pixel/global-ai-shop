...
# ── Customer page — সম্পূর্ণ standalone HTML ──────────────
@app.get("/call")
def call_page(request: Request, shop: str=""):
    # Supabase থেকে shop info নাও
    SUPA_URL = os.environ.get("SUPABASE_URL","")
    SUPA_KEY = os.environ.get("SUPABASE_KEY","")
    biz_name = "কাস্টমার কেয়ার"
    shop_ctx = ""
    if SUPA_URL and SUPA_KEY and shop:
        try:
            h={"apikey":SUPA_KEY,"Authorization":f"Bearer {SUPA_KEY}","Content-Type":"application/json"}
            r=requests.get(f"{SUPA_URL}/rest/v1/merchant_data?merchant_phone=eq.{shop}&select=rules,inventory,profile",headers=h,timeout=8)
            if r.status_code==200 and r.json():
                ...
                prods=" | ".join([p.get("name","")+": "+p.get("desc","") for p in inv[:10]])
    ...
    # voice API same server port 8000
...
padding:30px 22px 26px;width:100%;max-width:340px;
...
.err{{color:#ff8080;font-size:.72rem;margin-top:8px;text-align:center;
display:none;padding:6px 10px;background:rgba(255,59,48,.1);border-radius:8px;width:100%;}}
</style>
</head>
<body>
<div class="card">
  ...
  <div class="st" id="st">কল করতে নিচের বাটন চাপুন</div>
...
function playRing(){{
  var c=getAC();
  function tone(f,s,d){{var o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);
    o.type="sine";o.frequency.value=f;var t=c.currentTime+s;
...
async function apiCall(ep,msg){{
  try{{
    var r=await fetch(VAPI+"/"+ep,{{method:"POST",
...
function startListen(){{
  ...
  if(!SR){{setSt("Chrome এ খুলুন",false);showErr("Speech Recognition নেই");return;}}
  try{{
    ...
    rec.onend=function(){{showL(false);if(active&&!busy)setTimeout(startListen,500);}};
    rec.onerror=function(ev){{
      showL(false);
      if(ev.error==="not-allowed"){{showErr("মাইক্রোফোন Allow করুন");active=false;endCall();return;}}
