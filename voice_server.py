...
function speak(data){
  ...
  if(data.audio&&data.audio.length>100){
    ...
    if(window.speechSynthesis){
      ...
      var u=new SpeechSynthesisUtterance(data.reply||"জ্বি বলুন।");
  ...
  try{
    ...
    rec.onerror=function(ev){
      ...
      if(ev.error==="not-allowed"){
        showErr("মাইক্রোফোন Allow করুন:\\nBrowser এর address bar এ ক্লিক করুন → Allow করুন");
...
function startCall(){
  getAC(); // AudioContext unlock — user gesture এ
  ...
  // ২ সেকেন্ড delay — realistic ring
  setTimeout(function(){
    stopRing();
    active=true;
    ...
    // AI greeting — কল কানেক্ট হলে সাথে সাথে সালাম দেবে
...
function endCall(){
  ...
  setSt("কল শেষ হয়েছে। আবার করতে পারেন।",false);
