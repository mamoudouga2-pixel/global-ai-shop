...
def _get_secret(key, default=""):
    """Hugging Face secrets + os.environ দুটো থেকেই পড়ে"""
    try:
...
# ══════════════════════════════════════════════════════════
# VOICE MODULE — gTTS + Streamlit Audio + JS Bridge
# ══════════════════════════════════════════════════════════
...
_SALES_PROMPT = (
    ...
    "গুরুত্বপূর্ণ নিয়ম:\n"
    "১. তুমি কখনো বলবে না যে তুমি AI, রোবট বা অ্যাসিস্ট্যান্ট — তুমি একজন মানুষ কর্মী।\n"
    "২. কেউ সালাম দিলে আগে সালামের উত্তর দাও: 'ওয়ালাইকুম আসসালাম'।\n"
    "৩. কথা বলার ধরন: জ্বি ভাই, জ্বি আপু, জ্বি স্যার — এভাবে বিনয়ের সাথে কথা বলো।\n"
    "৪. সর্বোচ্চ ৩-৪ বাক্যে উত্তর দাও। সবসময় বাংলায় কথা বলো।\n"
    "৫. কাস্টমার একটা পণ্য চাইলে আরেকটা সম্পর্কিত পণ্যও suggest করো।\n"
    "৬. অর্ডার নিশ্চিত করতে নাম ও ঠিকানা চাও।\n"
    "৭. কণ্ঠস্বর উষ্ণ ও আন্তরিক রাখো — যেন একজন পরিচিত দোকানদার কথা বলছে।"
)
...
def get_ai_sales_response(user_message, shop_context=""):
    ...
    if gkey:
        try:
            ...
            if r.status_code == 200:
                ...
                txt = re.sub(r"(রিনা আপা|রিনা|AI assistant|আমি একটি AI)[,:]?\s*", "", txt)
                return txt
    ...
    if grkey:
        try:
            ...
            print("[Groq]", ex)
    return random.choice(["জ্বি বলুন, কীভাবে সাহায্য করতে পারি?", "হ্যাঁ ভাইয়া, বলুন।"])
...
def whatsapp_call_ui(shop_phone="", shop_context=""):
    ...
    for k, v in [("call_active",False),("call_greeted",False),
    ...
    if st.session_state.call_active and not st.session_state.call_greeted:
        with st.spinner("সংযুক্ত হচ্ছে..."):
            greet = get_ai_sales_response(
                "আসসালামু আলাইকুম বলো, তারপর নিজেকে কাস্টমার কেয়ার হিসেবে পরিচয় দাও এবং সাহায্যের কথা জিজ্ঞেস করো।",
                shop_context)
    ...
    call_btn  = f'''<button class="btn-c" id="callBtn" onclick="doCall()">{CALL_SVG}</button>'''
    end_btn   = f'''<button class="btn-e" onclick="doEnd()">{END_SVG}</button>'''

    top_section = """<div class="st">কল করতে নিচের বাটন চাপুন</div>""" if not is_active else """<div class="st"><span class="sdot"></span> সংযুক্ত</div><div class="tmr" id="tmr">00:00</div><div class="pulse"><span></span><span></span><span></span></div>"""
    btn_section = call_btn if not is_active else end_btn
    av_class    = "av ring" if not is_active else "av"
...
(function(){{
  ...
  try{{
    var b=atob("{audio_b64}"),buf=new ArrayBuffer(b.length),v=new Uint8Array(buf);
    for(var i=0;i<b.length;i++)v[i]=b.charCodeAt(i);
    var ctx=new(window.AudioContext||window.webkitAudioContext)();
    ctx.decodeAudioData(buf,function(d){{
      var s=ctx.createBufferSource();s.buffer=d;s.connect(ctx.destination);
      s.onended=startListen;s.start(0);
    }},startListen);
  }}catch(e){{startListen();}}
}})();
</script>'''
...
body{{font-family:"Noto Sans Bengali",sans-serif;background:transparent;display:flex;justify-content:center;padding:8px;}}
.wrap{{display:flex;flex-direction:column;align-items:center;background:linear-gradient(180deg,#0d1117,#161b22);border-radius:26px;padding:28px 20px 22px;width:100%;max-width:320px;box-shadow:0 8px 40px rgba(0,0,0,.7);border:1px solid rgba(255,255,255,.07);}}
.av{{width:86px;height:86px;border-radius:50%;background:linear-gradient(135deg,#25d366,#075e35);display:flex;align-items:center;justify-content:center;font-size:2.6rem;margin-bottom:12px;}}
.av.ring{{animation:rp 1.2s ease infinite;}}
@keyframes rp{{0%,100%{{box-shadow:0 0 0 0 rgba(37,211,102,.7);}}70%{{box-shadow:0 0 0 22px rgba(37,211,102,0);}}}}
...
.sdot{{width:7px;height:7px;border-radius:50%;background:#25d366;display:inline-block;animation:bk 1s infinite alternate;}}
@keyframes bk{{from{{opacity:.2;}}to{{opacity:1;}}}}
.tmr{{color:rgba(255,255,255,.35);font-size:.8rem;margin-bottom:12px;font-variant-numeric:tabular-nums;}}
.pulse{{display:flex;gap:5px;margin-bottom:14px;align-items:center;}}
.pulse span{{width:6px;height:6px;border-radius:50%;background:#25d366;animation:pl .7s ease infinite alternate;}}
.pulse span:nth-child(2){{animation-delay:.2s;}}.pulse span:nth-child(3){{animation-delay:.4s;}}
@keyframes pl{{from{{opacity:.2;transform:scale(.6);}}to{{opacity:1;transform:scale(1.4);}}}}
.btn-c{{width:64px;height:64px;border-radius:50%;background:#25d366;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 18px rgba(37,211,102,.55);animation:pg 1.6s ease infinite;}}
@keyframes pg{{0%,100%{{box-shadow:0 0 0 0 rgba(37,211,102,.5);}}70%{{box-shadow:0 0 0 16px rgba(37,211,102,0);}}}}
.btn-e{{width:64px;height:64px;border-radius:50%;background:#ff3b30;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(255,59,48,.5);}}
</style></head><body>
<div class="wrap">
  <div class="{av_class}">🏪</div>
  <div class="nm">কাস্টমার কেয়ার</div>
  {top_section}
  {btn_section}
</div>
<script>
var IS_ACTIVE={str(is_active).lower()};
var recog=null,callSec=0,callTimer=null;
function getBase(){{try{{return window.top.location.href.split("?")[0];}}catch(e){{return window.location.href.split("?")[0];}}}}
function nav(url){{try{{window.top.location.href=url;}}catch(e){{window.location.href=url;}}}}
function doCall(){{
  var btn=document.getElementById("callBtn");
  if(btn){{btn.disabled=true;btn.style.opacity=".5";}}
  try{{
    var ctx=new(window.AudioContext||window.webkitAudioContext)();
    function t(f,s,d){{var o=ctx.createOscillator(),g=ctx.createGain();o.connect(g);g.connect(ctx.destination);o.type="sine";o.frequency.value=f;g.gain.setValueAtTime(0,ctx.currentTime+s);g.gain.linearRampToValueAtTime(.1,ctx.currentTime+s+.06);g.gain.setValueAtTime(.1,ctx.currentTime+s+d-.06);g.gain.linearRampToValueAtTime(0,ctx.currentTime+s+d);o.start(ctx.currentTime+s);o.stop(ctx.currentTime+s+d+.1);}}
    function ring(){{t(440,0,.4);t(480,0,.4);t(440,.55,.4);t(480,.55,.4);}}
    ring();
    window._rt=setInterval(function(){{var c2=new(window.AudioContext||window.webkitAudioContext)();var o=c2.createOscillator(),g=c2.createGain();o.connect(g);g.connect(c2.destination);o.type="sine";o.frequency.value=440;g.gain.setValueAtTime(.1,c2.currentTime);g.gain.linearRampToValueAtTime(0,c2.currentTime+.4);o.start();o.stop(c2.currentTime+.5);setTimeout(function(){{var o2=c2.createOscillator(),g2=c2.createGain();o2.connect(g2);g2.connect(c2.destination);o2.type="sine";o2.frequency.value=480;g2.gain.setValueAtTime(.1,c2.currentTime+.6);g2.gain.linearRampToValueAtTime(0,c2.currentTime+1);o2.start(c2.currentTime+.6);o2.stop(c2.currentTime+1.1);}},0);}},2200);
  }}catch(e){{}}
  setTimeout(function(){{
...
function sendVoice(t){{
  if(recog){{try{{recog.stop();}}catch(e){{}}}}
  nav(getBase()+"?vr="+encodeURIComponent(t));
}}
function startListen(){{
  ...
  if(!SR)return;
  try{{
    recog=new SR();recog.lang="bn-BD";recog.interimResults=false;recog.continuous=false;
    recog.onresult=function(e){{var txt="";for(var i=e.resultIndex;i<e.results.length;i++)if(e.results[i].isFinal)txt+=e.results[i][0].transcript;if(txt.trim())sendVoice(txt.trim());}};
    recog.onend=function(){{if(IS_ACTIVE)setTimeout(startListen,1500);}};
    recog.onerror=function(ev){{if(ev.error!=="aborted"&&ev.error!=="no-speech")setTimeout(startListen,2000);}};
    recog.start();
  }}catch(e){{setTimeout(startListen,2000);}}
}}
if(IS_ACTIVE){{
  callTimer=setInterval(function(){{callSec++;var m=String(Math.floor(callSec/60)).padStart(2,"0"),s=String(callSec%60).padStart(2,"0");var e=document.getElementById("tmr");if(e)e.textContent=m+":"+s;}},1000);
}}
</script>
{audio_script}
</body></html>"""

    _c.html(html, height=360, scrolling=False)
...
def save_profile(phone, profile):
    """profile = {biz_name, logo_path, support_phone}"""
    if supabase:
        try:
            supabase.table("merchant_data").update({"profile": profile}).eq("merchant_phone", phone).execute()
        except: pass
...
def customer_page(shop_phone):
    """সম্পূর্ণ standalone HTML page — কোনো iframe নেই"""
    import json as _json

    products = load_inventory(shop_phone)
    profile  = load_profile(shop_phone)
    biz_name = profile.get("biz_name", "") or "কাস্টমার কেয়ার"
    gkey     = _get_secret("GEMINI_API_KEY") or GEMINI_API_KEY

    # Product list for AI context
    prod_text = ""
    for p in products:
        prod_text += f"{p.get('name','')} - {p.get('desc','')}\n"

    rules = load_user_memory(shop_phone)
    ai_system = (
        _SALES_PROMPT +
        f"\n\n[দোকানের নাম]: {biz_name}" +
        f"\n[নিয়ম]: {rules[:200]}" +
        f"\n[পণ্য]: {prod_text[:300]}"
    )

    # Escape for JS
    ai_sys_js = ai_system.replace("\\", "\\\\").replace("`", "\\`").replace("${", "\\${")
...
body{{
  ...
  min-height:100vh;
  display:flex;flex-direction:column;align-items:center;justify-content:center;
  background-image:radial-gradient(ellipse 70% 50% at 50% 0%, rgba(37,211,102,0.08) 0%,transparent 60%);
}}
.card{{
...
.shop-name{{color:rgba(255,255,255,.35);font-size:.75rem;letter-spacing:2px;text-transform:uppercase;margin-bottom:20px;}}
.avatar{{
...
.avatar.ring{{animation:rp 1.1s ease infinite;}}
@keyframes rp{{
  0%,100%{{box-shadow:0 0 0 0 rgba(37,211,102,.8);}}
  70%{{box-shadow:0 0 0 24px rgba(37,211,102,0);}}
}}
.avatar.active{{box-shadow:0 0 0 4px rgba(37,211,102,.4);}}
.name{{color:#fff;font-size:1.3rem;font-weight:700;margin-bottom:6px;}}
.status{{
  font-size:.82rem;color:rgba(255,255,255,.4);
  margin-bottom:20px;min-height:22px;text-align:center;
  display:flex;align-items:center;gap:6px;
}}
.dot{{width:8px;height:8px;border-radius:50%;background:#25d366;display:none;animation:bk .9s infinite alternate;}}
@keyframes bk{{from{{opacity:.2;}}to{{opacity:1;}}}}
.timer{{color:rgba(255,255,255,.3);font-size:.8rem;margin-bottom:16px;font-variant-numeric:tabular-nums;display:none;}}
.pulse{{display:none;gap:6px;margin-bottom:18px;align-items:center;}}
.pulse span{{width:7px;height:7px;border-radius:50%;background:#25d366;animation:pl .65s ease infinite alternate;}}
.pulse span:nth-child(2){{animation-delay:.2s;}}.pulse span:nth-child(3){{animation-delay:.4s;}}
@keyframes pl{{from{{opacity:.15;transform:scale(.5);}}to{{opacity:1;transform:scale(1.5);}}}}
.listening-text{{color:#25d366;font-size:.78rem;display:none;margin-bottom:12px;animation:bk .9s infinite alternate;}}
.btn-call{{
  width:72px;height:72px;border-radius:50%;
  background:#25d366;border:none;cursor:pointer;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 6px 24px rgba(37,211,102,.6);
  animation:pg 1.8s ease infinite;
  transition:transform .15s;
}}
.btn-call:active{{transform:scale(.93);}}
@keyframes pg{{0%,100%{{box-shadow:0 0 0 0 rgba(37,211,102,.6);}}70%{{box-shadow:0 0 0 18px rgba(37,211,102,0);}}}}
.btn-end{{
  width:72px;height:72px;border-radius:50%;
  background:#ff3b30;border:none;cursor:pointer;
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 6px 22px rgba(255,59,48,.55);display:none;
  transition:transform .15s;
}}
.btn-end:active{{transform:scale(.93);}}
.btn-end svg,.btn-call svg{{width:30px;height:30px;fill:white;}}
.call-label{{color:rgba(255,255,255,.3);font-size:.7rem;margin-top:8px;}}
</style>
</head>
<body>
<div class="card">
  <div class="shop-name" id="shopName">{biz_name}</div>
  <div class="avatar" id="avatar">🏪</div>
  <div class="name">কাস্টমার কেয়ার</div>
  <div class="status" id="status">
    <span class="dot" id="dot"></span>
    <span id="statusText">কল করতে নিচের বাটন চাপুন</span>
  </div>
  <div class="timer" id="timer">00:00</div>
  <div class="pulse" id="pulse"><span></span><span></span><span></span></div>
  <div class="listening-text" id="listenText">🎤 শুনছি...</div>
  ...
  <div class="call-label" id="callLabel">📞 কল করুন</div>
</div>

<script>
var GEMINI_KEY = "{gkey}";
var AI_SYSTEM  = `{ai_sys_js}`;
...
function playRingTone(){{
  ...
  tone(440,0,0.4); tone(480,0.05,0.4);
  tone(440,0.6,0.4); tone(480,0.65,0.4);
...
function startCall(){{
  ...
  setStatus("রিং হচ্ছে...", false);
  playRingTone();

  // ১৫-২২ সেকেন্ড পর রিসিভ
  var delay = 15000 + Math.random() * 7000;
  setTimeout(function(){{
    stopRing();
    callActive = true;
...
function endCall(){{
  ...
  $("callLabel").style.display = "block";
  $("callLabel").textContent = "📞 আবার কল করুন";
  $("avatar").className = "avatar";
  $("timer").style.display = "none";
  $("pulse").style.display = "none";
  $("listenText").style.display = "none";
  setStatus("কল শেষ হয়েছে", false);
  conversationHistory = [];
  callSec = 0;
}}

async function aiGreet(){{
  var greeting = "আসসালামু আলাইকুম, কেমন আছেন? আমি কাস্টমার কেয়ার থেকে বলছি, কীভাবে সাহায্য করতে পারি?";
...
async function callGemini(userText){{
  ...
  try{{
    var messages = [{{"role":"user","parts":[{{"text":AI_SYSTEM+"\n\nকাস্টমার: "+userText}}]}}];
    if(conversationHistory.length > 2){{
      messages = conversationHistory.slice(-6);
      messages[0].parts[0].text = AI_SYSTEM + "\n\n" + messages[0].parts[0].text;
    }}
    var res = await fetch(
      "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-lite:generateContent?key="+GEMINI_KEY,
      {{method:"POST",headers:{{"Content-Type":"application/json"}},
       body:JSON.stringify({{"contents":messages,"generationConfig":{{"temperature":0.9,"maxOutputTokens":120}}}})}}
    );
    var data = await res.json();
    var reply = data.candidates[0].content.parts[0].text.trim();
    reply = reply.replace(/\[EXTRACTED_DATA:.*?\]/gs,"").trim();
    conversationHistory.push({{"role":"model","parts":[{{"text":reply}}]}});
    return reply;
  }}catch(e){{
    return "জ্বি বলুন, শুনতে পাচ্ছি।";
  }}
}}
...
async function speakText(text){{
  ...
  return new Promise(function(resolve){{
    ...
    if(window.speechSynthesis){{
      ...
      var bnVoice = voices.find(function(v){{return v.lang.startsWith("bn")||v.lang.startsWith("hi");}});
      ...
      utt.onend = function(){{
        ...
        if(callActive){{
          ...
          $("listenText").style.display = "block";
          startListening();
        }}
        resolve();
      }};
...
function startListening(){{
  ...
  var SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if(!SR){{ setStatus("Chrome ব্রাউজারে খুলুন", false); return; }}
  try{{
    ...
    recog.onresult = function(e){{
      ...
      if(txt.trim()){{
        setStatus("বুঝছি...", true);
        callGemini(txt.trim()).then(function(reply){{
          speakText(reply);
        }});
      }}
    }};
    ...
    recog.onend = function(){{
      if(callActive && !isSpeaking) setTimeout(startListening, 800);
    }};

    recog.onerror = function(ev){{
      if(ev.error!=="aborted"&&ev.error!=="no-speech"){{
        setStatus("মাইক চালু করুন", false);
      }}
      if(callActive && !isSpeaking) setTimeout(startListening, 1500);
    }};
...
if(window.speechSynthesis){{
  ...
  window.speechSynthesis.getVoices();
    ...
    # সম্পূর্ণ HTML page হিসেবে render করো — কোনো Streamlit wrapper নেই
    import streamlit.components.v1 as _cv1
    _cv1.html(html_page, height=520, scrolling=False)
...
def dashboard():
    ...
    p_biz_name  = profile.get("biz_name", "") or biz_name
    p_support   = profile.get("support_phone", "")
    ...
    with st.sidebar:
        ...
        <div style='background:rgba(56,217,255,0.05);border:1px solid rgba(56,217,255,0.1);border-radius:14px;padding:14px 16px;margin-bottom:10px;'>
          <div style='display:flex;align-items:center;gap:10px;margin-bottom:8px;'>
            <div style='width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#38d9ff,#2563ff);display:flex;align-items:center;justify-content:center;font-size:1.4rem;'>🏪</div>
            <div>
              <p style='color:#e2eeff;font-weight:700;font-size:0.9rem;margin:0;'>{p_biz_name}</p>
        ...
        # ── প্রোফাইল এডিট ───────────────────
        with st.expander("✏️ প্রোফাইল সম্পাদনা করুন"):
            new_biz = st.text_input("ব্যবসার নাম", value=p_biz_name, key="p_biz")
            new_sup = st.text_input("সাপোর্ট নম্বর", value=p_support, placeholder="01XXXXXXXXX", key="p_sup")
            if st.button("💾 প্রোফাইল সেভ করুন", key="save_profile"):
                save_profile(owner_phone, {"biz_name": new_biz, "support_phone": new_sup})
                st.success("✅ প্রোফাইল আপডেট হয়েছে!")
                time.sleep(0.5); st.rerun()

        # ── কাস্টমার লিংক ───────────────────
        st.markdown("<p style='font-size:0.7rem;color:rgba(140,180,255,0.45);text-transform:uppercase;letter-spacing:1.5px;margin:10px 0 5px;'>🔗 কাস্টমার লিংক</p>", unsafe_allow_html=True)
        st.markdown(f"""
        <div class='link-box' onclick="navigator.clipboard.writeText('{short_link}');this.textContent='✅ কপি হয়েছে!';setTimeout(()=>this.textContent='{short_link}',2000);" title="ক্লিক করলে কপি হবে">
          {short_link}
        </div>
        <p style='font-size:0.7rem;color:rgba(140,180,255,0.35);margin-top:4px;'>👆 ক্লিক করুন → কপি হবে → Facebook এ শেয়ার করুন</p>
        """, unsafe_allow_html=True)
    ...
    # ── MAIN AREA ────────────────────────────
    st.markdown(f"""
    <div class="page-header" style="padding-top:1rem;position:relative;">
      <div style="position:absolute;right:0;top:1rem;display:flex;align-items:center;gap:10px;">
        <div style="background:rgba(56,217,255,0.08);border:1px solid rgba(56,217,255,0.18);border-radius:12px;padding:8px 14px;cursor:pointer;"
             onclick="document.getElementById('profile_panel').style.display=document.getElementById('profile_panel').style.display=='none'?'block':'none'">
          <span style="color:#38d9ff;font-size:.85rem;font-weight:700;">🏪 {p_biz_name}</span>
        </div>
      </div>
      <h1>GLOBAL AI</h1>
      <p>আপনার ব্যবসার সম্পূর্ণ AI কন্ট্রোল প্যানেল</p>
    </div>
    <div id="profile_panel" style="display:none;background:rgba(10,18,36,0.95);border:1px solid rgba(56,217,255,0.15);border-radius:16px;padding:20px;margin-bottom:14px;max-width:360px;margin-left:auto;">
      <p style="color:#38d9ff;font-weight:700;font-size:.9rem;margin-bottom:12px;">✏️ প্রোফাইল সেটিংস</p>
    </div>
    """, unsafe_allow_html=True)

    # profile edit in sidebar (always visible)
...
def main():
    ...
    # ── Voice call params — dashboard এ থাকলেও handle করো ──
    if "shop" in qp:
