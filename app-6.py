...
try:
    ...
    if _url.startswith("https://") and _key:
        try:
            ...
            _ver = tuple(int(x) for x in getattr(_sb_mod, "__version__", "0.0.0").split(".")[:2])
            if _ver >= (2, 0):
...
<body>
<div class="card">
  <div class="shop">BIZJS_PLACEHOLDER</div>
...
<script>
var CTX  = "CTXJS_PLACEHOLDER";
var VAPI = "VAPI_PLACEHOLDER";
...
if(window.speechSynthesis){
  ...
  window.speechSynthesis.onvoiceschanged=function(){window.speechSynthesis.getVoices();};
    ...
    html = html.replace("BIZJS_PLACEHOLDER", biz_js, 2)
    html = html.replace("CTXJS_PLACEHOLDER", ctx_js)
    html = html.replace("GKEYJS_PLACEHOLDER", gkey_js)
    html = html.replace("VAPI_PLACEHOLDER", VOICE_API_URL.rstrip("/"))
...
# Flask কে background এ চালাও — একবারই
import os as _os
if _os.environ.get("_FLASK_STARTED") != "1":
    _os.environ["_FLASK_STARTED"] = "1"
