...
# ─── দোকানের তথ্য Supabase থেকে আনো ───
def get_shop_info(shop_phone: str):
    ...
    if SUPA_URL and SUPA_KEY and shop_phone:
        try:
            ...
            r = requests.get(
                f"{SUPA_URL}/rest/v1/merchant_data"
                f"?merchant_phone=eq.{shop_phone}&select=rules,inventory,profile",
                headers=h, timeout=8
            )
...
# ─── AI রেসপন্স (Gemini → Groq fallback) ───
def get_reply(message: str, shop_context: str) -> str:
    prompt = SYSTEM + f"\n\n[দোকান]: {shop_context[:400]}\n\nকাস্টমার: {message}\n\nউত্তর:"
    if GEMINI_KEY:
        try:
            r = requests.post(
                f"https://generativelanguage.googleapis.com/v1beta"
                f"/models/gemini-2.0-flash-lite:generateContent?key={GEMINI_KEY}",
                json={
...
# ─── Text → Bengali Audio (base64) ───
def to_audio(text: str) -> str:
    try:
...
def greet(req: Req):
    msg = (
        "কাস্টমার কল করেছে। "
        "আন্তরিকভাবে আসসালামু আলাইকুম বলো, "
        "কাস্টমার কেয়ার হিসেবে পরিচয় দাও, "
        "কীভাবে সাহায্য করতে পারো জিজ্ঞেস করো।"
    )
...
# ════════════════════════════════════════════════════════════
#  /call?shop=PHONE  ←  এটাই কাস্টমারের লিংক
#  কোনো লগইন নেই, সরাসরি কল UI দেখায়
# ════════════════════════════════════════════════════════════
@app.get("/call")
...
<title>{biz_js} | কাস্টমার কেয়ার</title>
...
.nm{{color:#fff;font-size:1.2rem;font-weight:700;margin-bottom:4px;text-align:center;}}
.sub{{color:rgba(140,180,255,.4);font-size:.7rem;margin-bottom:16px;text-align:center;}}
...
display:none;padding:10px 14px;background:rgba(255,59,48,.08);
border-radius:10px;width:100%;line-height:1.7;}}

.info{{color:rgba(140,180,255,.3);font-size:.63rem;margin-top:14px;
...
<div class="card">
  ...
  <div class="sub">AI সহায়তা · ২৪/৭</div>
  ...
  <button class="bcall" id="bcall" onclick="startCall()" aria-label="কল করুন">
    <svg viewBox="0 0 24 24">
      <path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2
               1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1C8.6 21 1 13.4 1 4c0-.6.4-1 1-1h3.5
               c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.3 0 .7-.2 1L6.6 10.8z"/>
    </svg>
  </button>

  <button class="bend" id="bend" onclick="endCall()" aria-label="কল কাটুন">
    <svg viewBox="0 0 24 24">
      <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97
               c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.12-.56-2.3-.56-3.53
               0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21
  ...
  <div class="info" id="info">
    Chrome বা Edge browser এ সবচেয়ে ভালো কাজ করে<br>
    প্রথমবার মাইক্রোফোন permission দিতে হবে
  </div>
</div>
...
var VAPI = "{base_url}";
var CTX  = "{ctx_js}";
var active=false, rec=null, sec=0, tid=null, busy=false;
var audioCtx=null, ringTmr=null;
...
function showInfo(v){{$("info").style.display=v?"block":"none";}}
function showErr(t){{
  var e=$("err");
  if(t){{e.innerHTML=t;e.style.display="block";}}
  ...
  try{{
    ...
    c.decodeAudioData(buf,
      function(dec){{
        ...
        s.onended=function(){{
          busy=false;showP(false);
          if(active){{setSt("বলুন...",true);showL(true);startListen();}}
        }};
        s.start(0);
      }},
...
function speak(data){{
  ...
  if(data.audio&&data.audio.length>100){{
    ...
    speakFallback(data.reply||"জ্বি বলুন।");
  }}
}}

async function apiCall(ep,msg){{
  try{{
    var r=await fetch(VAPI+"/"+ep,{{
      method:"POST",
      headers:{{"Content-Type":"application/json"}},
      body:JSON.stringify({{message:msg,shop_context:CTX}})
    }});
    if(!r.ok) throw new Error("HTTP "+r.status);
    return await r.json();
  }}catch(e){{
    console.error("[API]",e);
    return {{reply:"জ্বি বলুন, কীভাবে সাহায্য করতে পারি?",audio:""}};
  }}
}}
...
function startListen(){{
  ...
  if(!SR){{
    setSt("Chrome/Edge ব্যবহার করুন",false);
    showErr("এই browser এ voice সাপোর্ট নেই।<br>Chrome বা Edge browser এ খুলুন।");
    return;
  }}
  try{{
    ...
    rec.onerror=function(ev){{
      ...
      if(ev.error==="not-allowed"||ev.error==="permission-denied"){{
        showErr(
          "⚠️ মাইক্রোফোন Allow করুন<br><br>"+
          "Browser এর address bar এ 🔒 আইকনে ক্লিক করুন → Allow<br>"+
          "তারপর পেজ Reload করুন"
        );
        active=false;endCall();return;
      }}
      if(ev.error==="network"){{showErr("নেটওয়ার্ক সমস্যা। ইন্টারনেট চেক করুন।");}}
...
// ✅ কল শুরু — ২ সেকেন্ড রিং → AI সালাম দেবে
function startCall(){{
  getAC();
  showErr("");showInfo(false);
  ...
  setTimeout(function(){{
    ...
    // ✅ AI নিজে থেকে সালাম দিয়ে কথা শুরু করবে
