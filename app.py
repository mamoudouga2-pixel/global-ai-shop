...
def main():
    # ════════════════════════════════════════════════════════
    #  ১. কাস্টমার লিংক চেক — ?shop=PHONE থাকলে
    #     সরাসরি Voice Server এ পাঠাও, লগইন দেখাবে না
    # ════════════════════════════════════════════════════════
    qp = st.query_params
    if "shop" in qp:
        shop = str(qp["shop"]).strip()
        redirect_url = VOICE_API_URL.rstrip("/") + "/call?shop=" + shop
        # সম্পূর্ণ পেজ লোড হওয়ার আগেই redirect করো
        st.markdown(f"""
<style>
  body, .stApp {{
  ...
  header, footer, #root > div:first-child {{
...
<script>
(function() {{
  try {{ window.top.location.replace("{redirect_url}"); }}
  catch(e) {{ window.location.replace("{redirect_url}"); }}
}})();
</script>
<div style="
...
">
  ...
  <p style="color:#25d366;font-size:1.1rem;font-weight:700;margin-bottom:8px;">সংযুক্ত হচ্ছে...</p>
  <p style="color:rgba(255,255,255,.3);font-size:.8rem;margin-bottom:24px;">কাস্টমার কেয়ারে কল করা হচ্ছে</p>
  <a href="{redirect_url}"
     style="color:#25d366;font-size:.85rem;padding:10px 24px;
