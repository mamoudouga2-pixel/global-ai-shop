...
VOICE_API_URL = BASE_URL.rstrip("/") + ":8000"
...
def main():
    qp = st.query_params
    if "shop" in qp:
        # FastAPI voice server এ redirect করো
        shop = qp["shop"]
        redirect_url = f"{VOICE_API_URL}/call?shop={shop}"
        st.markdown(f"""
        <meta http-equiv="refresh" content="0; url={redirect_url}">
        <script>window.location.href="{redirect_url}";</script>
        <p style="color:white;text-align:center;margin-top:40px;">
        সংযুক্ত হচ্ছে... <a href="{redirect_url}" style="color:#25d366;">এখানে ক্লিক করুন</a></p>
        """, unsafe_allow_html=True)
        return
