...
def _get_secret(key, default=""):
    try:
        if hasattr(st, "secrets") and hasattr(st.secrets, "_secrets") and key in st.secrets:
...
<div style="color:rgba(255,150,150,.5);font-size:.8rem;">ব্যবসায়ীকে জানান — মেয়াদ বাড়াতে হবে।</div>
    ...
    html = (
        ...
        "</head>"
        "<body style='margin:0;padding:0;background:#010409;font-family:Noto Sans Bengali,sans-serif;'>"
        "<div id='app'></div>"
        "<script>"
        f"var CTX = {repr(ctx_js)};"
        f"var GKEY = {repr(gkey_js)};"
        ...
        "var active=false,busy=false,rec=null,sec=0,tid=null,audioCtx=null,ringTmr=null;"
        ...
        "function playRing(){var c=getAC();function tone(f,s,d){var o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type='sine';o.frequency.value=f;var t=c.currentTime+s;g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(.12,t+.06);g.gain.setValueAtTime(.12,t+d-.06);g.gain.linearRampToValueAtTime(0,t+d);o.start(t);o.stop(t+d+.1);}function ring(){tone(440,0,.45);tone(480,.05,.45);tone(440,.72,.45);tone(480,.77,.45);}ring();ringTmr=setInterval(ring,2800);}"
        "function stopRing(){if(ringTmr){clearInterval(ringTmr);ringTmr=null;}}"
        "function speakText(text){if(!window.speechSynthesis){busy=false;if(active)startListen();return;}window.speechSynthesis.cancel();var u=new SpeechSynthesisUtterance(text||'জ্বি বলুন।');u.lang='bn-IN';u.rate=0.9;u.volume=1;var vs=window.speechSynthesis.getVoices();var bn=vs.find(function(v){return v.lang&&(v.lang.startsWith('bn')||v.lang.startsWith('hi'));});if(bn)u.voice=bn;u.onend=function(){busy=false;if(active){setSt('বলুন...',true);startListen();}};u.onerror=function(){busy=false;if(active)startListen();};window.speechSynthesis.speak(u);}"
        "async function gemini(msg){try{var prompt=SYS+'\n\nকাস্টমার: '+msg+'\n\nউত্তর:';var r=await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-lite:generateContent?key='+GKEY,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.85,maxOutputTokens:100}})});if(!r.ok)throw new Error('HTTP '+r.status);var d=await r.json();return d.candidates[0].content.parts[0].text.trim();}catch(e){return 'জ্বি বলুন, কীভাবে সাহায্য করতে পারি?';}}"
        "function setSt(t,g){$('st').textContent=t;$('st').style.color=g?'#25d366':'rgba(255,255,255,.5)';}"
        "function startListen(){if(!active||busy)return;var SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){setSt('Chrome এ খুলুন',false);return;}try{if(rec){try{rec.abort();}catch(e){}}rec=new SR();rec.lang='bn-IN';rec.continuous=false;rec.interimResults=true;rec.maxAlternatives=1;rec.onstart=function(){setSt('বলুন...',true);};rec.onresult=function(e){var txt='',fin=false;for(var i=e.resultIndex;i<e.results.length;i++){if(e.results[i].isFinal){txt+=e.results[i][0].transcript;fin=true;}else setSt('শুনছি: '+e.results[i][0].transcript,true);}if(fin&&txt.trim()){setSt('বুঝছি...',false);if(rec){try{rec.abort();}catch(ex){}}busy=true;gemini(txt.trim()).then(function(reply){setSt('বলছে...',true);speakText(reply);});}};rec.onend=function(){if(active&&!busy)setTimeout(startListen,600);};rec.onerror=function(ev){if(ev.error==='not-allowed'){setSt('মাইক Allow করুন',false);active=false;endCall();return;}if(active&&!busy)setTimeout(startListen,1200);};rec.start();}catch(e){if(active&&!busy)setTimeout(startListen,1500);}}"
        "function startCall(){getAC();$('bcall').style.display='none';$('av').style.animation='rp 1.1s ease infinite';setSt('রিং হচ্ছে...',false);$('lbl').textContent='সংযুক্ত হচ্ছে...';playRing();setTimeout(function(){stopRing();active=true;$('av').style.animation='none';$('av').style.boxShadow='0 0 0 6px rgba(37,211,102,.3)';$('bend').style.display='flex';$('tmr').style.display='block';$('lbl').textContent='📵 কাটুন';setSt('সংযুক্ত',true);tid=setInterval(function(){sec++;var m=String(Math.floor(sec/60)).padStart(2,'0'),s=String(sec%60).padStart(2,'0');$('tmr').textContent=m+':'+s;},1000);busy=true;gemini('কাস্টমার এইমাত্র কল করেছে। আন্তরিকভাবে আসসালামু আলাইকুম বলো, কাস্টমার কেয়ার হিসেবে পরিচয় দাও, কীভাবে সাহায্য করতে পারো জিজ্ঞেস করো।').then(function(reply){setSt('বলছে...',true);speakText(reply);});},2000);}"
        "function endCall(){active=false;busy=false;stopRing();if(tid){clearInterval(tid);tid=null;}if(rec){try{rec.abort();}catch(e){}}if(window.speechSynthesis)window.speechSynthesis.cancel();$('av').style.animation='none';$('av').style.boxShadow='none';$('bend').style.display='none';$('tmr').style.display='none';$('tmr').textContent='00:00';sec=0;$('bcall').style.display='flex';$('lbl').textContent='📞 কল করুন';setSt('কল করতে নিচের সবুজ বাটন চাপুন',false);}"
        "window.speechSynthesis&&window.speechSynthesis.getVoices();"
        "</script>"
        "<style>"
        "*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;}"
        "html,body{width:100%;min-height:100vh;background:#010409;overflow-x:hidden;}"
        "body{font-family:'Noto Sans Bengali',sans-serif;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:20px 16px 40px;background-image:radial-gradient(ellipse 70% 50% at 50% 0%,rgba(37,211,102,.09) 0%,transparent 60%);}"
        ".card{background:linear-gradient(180deg,#0d1117,#161b22);border-radius:24px;padding:30px 20px 26px;width:100%;max-width:340px;box-shadow:0 8px 40px rgba(0,0,0,.8);border:1px solid rgba(255,255,255,.07);display:flex;flex-direction:column;align-items:center;}"
        f".shop{{color:rgba(255,255,255,.25);font-size:.65rem;letter-spacing:3px;text-transform:uppercase;margin-bottom:20px;}}"
        ".av{width:90px;height:90px;border-radius:50%;background:linear-gradient(135deg,#25d366,#075e35);display:flex;align-items:center;justify-content:center;font-size:2.8rem;margin-bottom:12px;}"
        ...
        ".st{color:rgba(255,255,255,.5);font-size:.82rem;margin-bottom:6px;min-height:1.2em;text-align:center;transition:color .3s;}"
        ".tmr{color:#25d366;font-size:1.1rem;font-weight:700;margin-bottom:10px;display:none;letter-spacing:2px;}"
        ".pulse{display:none;gap:4px;margin-bottom:14px;align-items:center;}"
        ".pulse span{display:block;width:4px;border-radius:2px;background:#25d366;animation:pb .8s ease infinite;}"
        ".pulse span:nth-child(1){height:12px;animation-delay:0s;}"
        ".pulse span:nth-child(2){height:24px;animation-delay:.15s;}"
        ".pulse span:nth-child(3){height:12px;animation-delay:.3s;}"
        "@keyframes pb{0%,100%{transform:scaleY(1);}50%{transform:scaleY(1.8);}}"
        ".bcall{width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,#25d366,#1da851);border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(37,211,102,.5);margin-bottom:10px;transition:transform .15s;}"
        ".bcall:active{transform:scale(.92);}"
        ".bcall svg{width:30px;height:30px;fill:#fff;}"
        ".bend{width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#ff3b30,#c0392b);border:none;cursor:pointer;display:none;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(255,59,48,.4);margin-bottom:10px;}"
        ".bend svg{width:28px;height:28px;fill:#fff;}"
    ...
    _cv1.html(html, height=720, scrolling=False)
...
# Flask কে background এ চালাও — একবারই চালু হবে
import threading as _threading
_flask_started = getattr(_run_flask, "_started", False)
if not _flask_started:
    _run_flask._started = True
    _t = _threading.Thread(target=_run_flask, daemon=True)
    _t.start()
