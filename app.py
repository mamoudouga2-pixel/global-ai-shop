...
def whatsapp_call_ui(shop_phone="", shop_context=""):
    ...
    # ── সম্পূর্ণ WhatsApp UI + Web Speech API ─────────────
    html = f"""<!DOCTYPE html>
<html lang="bn"><head>
<meta charset="UTF-8">
...
body{{font-family:'Noto Sans Bengali',sans-serif;background:transparent;}}
.wrap{{display:flex;flex-direction:column;align-items:center;
  background:linear-gradient(180deg,#1a1a2e,#111);
  border-radius:24px;padding:32px 20px 28px;width:100%;max-width:360px;
  margin:0 auto;box-shadow:0 8px 40px rgba(0,0,0,.5);}}
...
.av.ring{{animation:rp 1.4s ease infinite;}}
@keyframes rp{{
  0%,100%{{box-shadow:0 0 0 0 rgba(37,211,102,.6),0 0 0 0 rgba(37,211,102,.3);}}
  50%{{box-shadow:0 0 0 18px rgba(37,211,102,.1),0 0 0 36px rgba(37,211,102,.04);}}
}}
.nm{{color:#fff;font-size:1.35rem;font-weight:700;margin-bottom:4px;}}
.st{{color:rgba(255,255,255,.4);font-size:.8rem;margin-bottom:20px;}}
.timer{{color:rgba(255,255,255,.4);font-size:.85rem;font-variant-numeric:tabular-nums;margin-bottom:16px;}}
.btn-row{{display:flex;gap:20px;align-items:center;justify-content:center;margin-top:8px;}}
.btn-call{{width:68px;height:68px;border-radius:50%;background:#25d366;border:none;
  cursor:pointer;display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 20px rgba(37,211,102,.6);}}
.btn-end{{width:68px;height:68px;border-radius:50%;background:#ff3b30;border:none;
  cursor:pointer;display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 20px rgba(255,59,48,.5);}}
.btn-call svg,.btn-end svg{{width:30px;height:30px;fill:white;}}
.ic-row{{display:flex;gap:24px;justify-content:center;margin-bottom:16px;}}
.ic-btn{{display:flex;flex-direction:column;align-items:center;gap:5px;
  border:none;background:none;cursor:pointer;}}
.ic{{width:54px;height:54px;border-radius:50%;background:rgba(255,255,255,.12);
  display:flex;align-items:center;justify-content:center;}}
.ic svg{{width:22px;height:22px;fill:white;}}
.ic.on{{background:rgba(255,255,255,.88);}}
.ic.on svg{{fill:#000;}}
.ic-lbl{{color:rgba(255,255,255,.4);font-size:.65rem;}}
...
.pulse span{{width:7px;height:7px;border-radius:50%;background:#25d366;
  animation:pl .7s ease infinite alternate;}}
.pulse span:nth-child(2){{animation-delay:.15s;}}
.pulse span:nth-child(3){{animation-delay:.3s;}}
@keyframes pl{{from{{opacity:.3;transform:scale(.7);}}to{{opacity:1;transform:scale(1.3);}}}}
</style></head><body>
<div class="wrap">
  <div class="av {"ring" if not is_active else ""}">🏪</div>
  <div class="nm">কাস্টমার কেয়ার</div>
  <div class="st" id="status">{"কল করতে বাটন চাপুন" if not is_active else "🟢 সংযুক্ত"}</div>
  {"<div class='timer' id='tmr'>00:00</div>" if is_active else ""}
  {"<div class='pulse' id='pulse'><span></span><span></span><span></span></div>" if is_active else ""}
  ...
  {"""
  <div class="ic-row">
    <button class="ic-btn" onclick="toggleSpk()">
      <div class="ic" id="spk_ic">
        <svg viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
      </div>
      <span class="ic-lbl">স্পিকার</span>
    </button>
    <button class="ic-btn" onclick="toggleMic()">
      <div class="ic" id="mic_ic">
      ...
      <span class="ic-lbl">মাইক</span>
    </button>
    <button class="ic-btn" onclick="endCall()">
      <div class="btn-end">
...
var muted=false, _actx=null, callSec=0, callTimer=null, recog=null;
...
function setSt(t){{var e=document.getElementById("status");if(e)e.textContent=t;}}
function setPulse(v){{var e=document.getElementById("pulse");if(e)e.style.display=v?"flex":"none";}}

function playRing(){{
  try{{
    var ctx=new(window.AudioContext||window.webkitAudioContext)();
    function b(c,f,s,d){{var o=c.createOscillator(),g=c.createGain();o.connect(g);g.connect(c.destination);o.type="sine";o.frequency.value=f;g.gain.setValueAtTime(0,c.currentTime+s);g.gain.linearRampToValueAtTime(0.13,c.currentTime+s+0.06);g.gain.setValueAtTime(0.13,c.currentTime+s+d-0.06);g.gain.linearRampToValueAtTime(0,c.currentTime+s+d);o.start(c.currentTime+s);o.stop(c.currentTime+s+d+0.05);}}
    function r(c){{b(c,480,0,.4);b(c,620,0,.4);b(c,480,.5,.4);b(c,620,.5,.4);}}
    r(ctx);window._rctx=ctx;
    window._ringT=setInterval(function(){{var c2=new(window.AudioContext||window.webkitAudioContext)();r(c2);window._rctx=c2;}},3500);
  }}catch(e){{}}
}}

function stopRing(){{
  if(window._ringT){{clearInterval(window._ringT);window._ringT=null;}}
  if(window._rctx){{try{{window._rctx.close();}}catch(e){{}}window._rctx=null;}}
}}

function playAudio(b64){{
  if(!b64||b64.length<10){{afterAudio();return;}}
  try{{
    var ctx=unlockAudio();
    ...
    ctx.decodeAudioData(buf,function(dec){{
      var src=ctx.createBufferSource();src.buffer=dec;src.connect(ctx.destination);
      src.onended=afterAudio;src.start(0);
      setSt("🟢 সংযুক্ত");setPulse(false);
    }},function(){{afterAudio();}});
  }}catch(e){{afterAudio();}}
}}

function afterAudio(){{setSt("বলুন...");setPulse(true);startListen();}}

function startTimer(){{
  callSec=0;
  callTimer=setInterval(function(){{
    callSec++;
    var m=String(Math.floor(callSec/60)).padStart(2,"0"),s=String(callSec%60).padStart(2,"0");
    var e=document.getElementById("tmr");if(e)e.textContent=m+":"+s;
  }},1000);
}}

function toggleSpk(){{document.getElementById("spk_ic").classList.toggle("on");}}
function toggleMic(){{
  muted=!muted;
  document.getElementById("mic_ic").classList.toggle("on");
  if(muted&&recog){{try{{recog.stop();}}catch(e){{}}}}
  else if(!muted){{startListen();}}
}}

// JS → Python: Streamlit query param দিয়ে rerun
function sendVoice(txt){{
  setSt("প্রসেস হচ্ছে...");setPulse(false);
  var url=new URL(window.location.href);
  url.searchParams.set("vr",txt);
...
function startCall(){{
  ...
  setSt("রিং হচ্ছে...");
  playRing();
  var delay=15000+Math.random()*8000;
  setTimeout(function(){{
    stopRing();
    // Streamlit কে জানাও — call শুরু
    var url=new URL(window.location.href);
...
function startListen(){{
  ...
  if(!SR){{setSt("Chrome ব্রাউজারে খুলুন");return;}}
  try{{
    ...
    recog.onend=function(){{if(!muted&&IS_ACTIVE)setTimeout(startListen,1200);}};
    recog.onerror=function(ev){{if(ev.error!=="aborted"&&ev.error!=="no-speech")setSt("মাইক্রোফোন অ্যাক্সেস দিন");}}; 
    recog.start();
  }}catch(e){{setSt("মাইক্রোফোন সমস্যা");}}
}}
...
window.onload=function(){{
  if(IS_ACTIVE){{
    ...
    if(AUDIO_B64&&AUDIO_B64.length>10){{
      setTimeout(function(){{playAudio(AUDIO_B64);}},200);
    }}else{{
      setSt("বলুন...");setPulse(true);startListen();
    }}
  }}
}};
</script>
    ...
    # Render এ সরাসরি চলে — কোনো iframe restriction নেই
    _c.html(html, height=420, scrolling=False)

    # ── Query param থেকে state update ────────────────────
    qp = st.query_params
    call_param  = qp.get("call", "")
    voice_param = qp.get("vr",   "").strip()

    if call_param == "1" and not st.session_state.call_active:
        try: del st.query_params["call"]
        except: pass
        st.session_state.call_active  = True
        st.session_state.call_greeted = False
        st.rerun()
    elif call_param == "0" and st.session_state.call_active:
        try: del st.query_params["call"]
        except: pass
        ...
        st.rerun()
    elif voice_param and voice_param not in ("", "None"):
        try: del st.query_params["vr"]
        except: pass
        st.session_state.voice_text = voice_param
        st.rerun()
