...
try:
    ...
    if _url.startswith("https://") and _key:
        try:
            ...
            supabase = create_client(_url, _key, options=ClientOptions(postgrest_client_timeout=15, storage_client_timeout=15))
        except Exception:
...
def get_or_create_user(phone):
    if supabase:
        try:
            ...
            if res.data:
                d = res.data[0]
                return (d.get("phone",""), d.get("business_name","My Business"), d.get("email",""), d.get("expiry_date","2099-12-31"), d.get("status","FREE"))
            supabase.table("merchants").insert({"phone": phone, "business_name": "My Business", "email": "", "expiry_date": "2099-12-31", "status": "FREE", "pin": hash_pass("0000")}).execute()
            try:
                supabase.table("merchant_data").insert({"merchant_phone": phone, "rules": "", "inventory": []}).execute()
            except Exception: pass
            return (phone, "My Business", "", "2099-12-31", "FREE")
        except Exception as e:
            print("[get_or_create_user]", e)
    return (phone, "My Business", "", "2099-12-31", "FREE")
...
def save_order(phone, order_data):
    if supabase:
        try:
            ...
            order_data["merchant_phone"] = phone
            order_data["order_date"] = _dt.datetime.now().isoformat()
            supabase.table("orders").insert(order_data).execute()
        except Exception as e: print("[save_order]", e)

def get_all_orders(phone):
    if supabase:
        try:
            res = supabase.table("orders").select("*").eq("merchant_phone", phone).execute()
            if res.data and pd: return pd.DataFrame(res.data)
        except Exception as e: print("[get_all_orders]", e)
    return None

def get_ai_response(msg, tone, key, ctx): return msg
...
if(window.speechSynthesis){
  ...
  window.speechSynthesis.onvoiceschanged=function(){window.speechSynthesis.getVoices();};
    ...
    html = html.replace("__BIZ__", biz_js).replace("__BIZ__", biz_js)
    html = html.replace("__CTX__", ctx_js)
    html = html.replace("__GKEY__", gkey_js)
    html = html.replace("__VAPI__", VOICE_API_URL.rstrip("/"))
...
# Flask একবারই চালু হবে
import os as _fos
if not _fos.environ.get("_FLASK_STARTED"):
    _fos.environ["_FLASK_STARTED"] = "1"
