...
VOICE_API_URL = os.environ.get("VOICE_API_URL", BASE_URL.rstrip("/") + ":8000")

def get_short_link(phone):
    # Customer লিংক এখন FastAPI page এ যাবে — Streamlit না
    return f"{VOICE_API_URL}/call?shop={phone}"
...
def customer_page(shop_phone):
    ...
    ctx_js = ctx.replace("\\","\\\\").replace("`","'").replace("$","").replace("\n"," ")
    biz_js = biz_name.replace("<","").replace(">","").replace("`","").replace("$","").replace("\n","")
    gkey_js = gkey.strip()
...
var GKEY = """" + gkey_js + """";
var CTX  = """" + ctx_js + """";
var SYS = "তুমি একজন বাংলাদেশী দোকানের কাস্টমার কেয়ার প্রতিনিধি। তুমি কখনো বলবে না যে তুমি AI বা রোবট। কেউ সালাম দিলে আগে ওয়ালাইকুম আসসালাম বলো। সালাম না দিলে সরাসরি উত্তর দাও। জ্বি ভাই/আপু বলো। সর্বোচ্চ ২-৩ বাক্যে উত্তর দাও। সবসময় বাংলায়। দোকান: " + CTX;

var active=false, rec=null, sec=0, tid=null, busy=false, audioCtx=null, ringTmr=null;

function $(i){return document.getElementById(i);}
function setSt(t,g){$("st").textContent=t;$("st").className="st"+(g?" green":"");}
...
function speakText(text){
  ...
  u.lang="bn-BD";u.rate=0.9;u.volume=1;
  var vs=window.speechSynthesis.getVoices();
  var bn=vs.find(function(v){return v.lang&&v.lang.startsWith("bn");});
  if(bn)u.voice=bn;
...
async function gemini(msg){
  setSt("ভাবছে...",false);showL(false);
  var prompt=SYS+"\n\nকাস্টমার: "+msg+"\n\nউত্তর:";
  try{
    var r=await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-lite:generateContent?key="+GKEY,{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({contents:[{parts:[{text:prompt}]}],generationConfig:{temperature:0.85,maxOutputTokens:80}})
    });
    var d=await r.json();
    return d.candidates[0].content.parts[0].text.trim().replace(/\[.*?\]/g,"").trim();
  }catch(e){return "জ্বি বলুন।";}
}

function speak(text){
  busy=true;showP(true);showL(false);setSt("🔊 বলছে...",true);
  // প্রথমে Web Speech try করো — fast
  speakText(text);
}

async function respond(msg){
  var reply=await gemini(msg);
  speak(reply);
}
...
function startListen(){
  ...
  if(!SR){setSt("Chrome এ খুলুন",false);showErr("Speech Recognition সাপোর্ট নেই");return;}
  try{
    if(rec){try{rec.abort();}catch(e){}}
    rec=new SR();
    // Android এ bn-IN ভালো কাজ করে
    var langs=["bn-IN","bn-BD","hi-IN"];
    rec.lang=langs[window._langIdx||0];
    ...
    rec.onresult=function(e){
      ...
      for(var i=e.resultIndex;i<e.results.length;i++){
        if(e.results[i].isFinal){txt+=e.results[i][0].transcript;isFinal=true;}
    ...
    rec.onend=function(){
      showL(false);
      if(active&&!busy)setTimeout(startListen,500);
    };
    rec.onerror=function(ev){
      showL(false);
      if(ev.error==="not-allowed"){
        showErr("মাইক্রোফোন permission দিন — browser settings এ Allow করুন");
        active=false;endCall();return;
      }
      if(ev.error==="language-not-supported"){
        // পরের language try করো
        window._langIdx=((window._langIdx||0)+1)%3;
        showErr("language চেষ্টা: "+langs[window._langIdx]);
      }
      if(active&&!busy)setTimeout(startListen,1000);
    };
    rec.start();
...
function startCall(){
  ...
  setTimeout(function(){
    ...
    // AI greeting
    respond("কাস্টমার এইমাত্র কল করেছে। আসসালামু আলাইকুম বলো, কাস্টমার কেয়ার হিসেবে পরিচয় দাও, সাহায্যের কথা জিজ্ঞেস করো।");
...
function endCall(){
  ...
  setSt("কল শেষ। আবার কল করতে পারেন।",false);
...
def main():
    qp = st.query_params
    # Customer লিংক — লগইন bypass, সরাসরি voice server এ
    if "shop" in qp:
        shop = qp["shop"]
        redirect_url = VOICE_API_URL + "/call?shop=" + shop
        st.markdown(
            f'''<meta http-equiv="refresh" content="0; url={redirect_url}">
<script>window.top.location.href="{redirect_url}";</script>
<style>body{{background:#010409;display:flex;align-items:center;justify-content:center;
min-height:100vh;margin:0;font-family:sans-serif;}}</style>
<p style="color:#25d366;font-size:1rem;text-align:center;">
সংযুক্ত হচ্ছে...<br><br>
<a href="{redirect_url}" style="color:#25d366;font-size:.85rem;">না গেলে এখানে ক্লিক করুন</a></p>''',
            unsafe_allow_html=True)
        st.stop()
        return
